...
type jobStore interface {
	Create(scope, idempotencyKey string, chat preparedChat) (*chatJob, bool)
...
type memoryJobStore struct {
	...
	requests map[string]preparedChat
}
...
func (s *memoryJobStore) Create(scope, idempotencyKey string, chat preparedChat) (*chatJob, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	job := &chatJob{
		...
		CreatedAt:      now,
		UpdatedAt:      now,
		Model:          chat.Req.Model,
		ResolvedModel:  chat.ResolvedModel,
		Runtime:        string(chat.Runtime),
		idempotencyKey: idempotencyKey,
		scope:          scope,
	}
	s.jobs[job.ID] = job
	s.requests[job.ID] = chat
	if idempotencyKey != "" {
...
func (s *memoryJobStore) request(id string) (preparedChat, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
